...
point3 lookat = glm::normalize(point3(0,0,-1));
point3 up = point3(0,1,0);
point3 cam_right = glm::cross(lookat, up);
point3 cam_up = glm::cross(cam_right, lookat);
float d = 1;
...
point3 s(int x, int y) {
	...
	//return glm::normalize(u*cam_right + v*cam_up + lookat);
    return u*cam_right + v*cam_up + lookat;
}
	...
	if (drawing_y <= 0.5) {
		...
		if (drawing_y == int(drawing_y)) {
			...
			for (int x = 0; x < vp_width; x++) {
				if (!trace(eye, eye+s(x, y), texture[x], false)) {
